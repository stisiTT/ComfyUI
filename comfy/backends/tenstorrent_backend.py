...
DEPRECATED: This Unix-socket bridge client is superseded by the HTTP client in
``comfy/backends/tt_http_client.py``, which talks to the maintained tt-metal
FastAPI server (``launch_server.sh``). New code should use ``tt_http_client``.
This module is retained only for backward compatibility with older bridge
deployments and may be removed in a future change.
